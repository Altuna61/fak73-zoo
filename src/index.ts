...
app.patch("/compounds/:id", async (c) => {
  ...
  if (!gehege) return console.error("Compound not found, dulli.");
...
app.patch("/animals/:id", async (c) => {
  ...
  if (!tier) return console.error("Compound not found, dulli.");
...
app.delete("/animals/:id", async (c) => {
  const { id } = c.req.param();
  const tier = await pool.query(`SELECT * FROM "Tier" WHERE id = $1`, [id]);
  if (!tier) return console.error("Tier nicht vorhanden.");
