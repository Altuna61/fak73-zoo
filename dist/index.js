...
var pool = new Pool({ ssl: { rejectUnauthorized: false } });
var app = new Hono2;
app.use("*", cors());
...
app.patch("/compounds/:id", async (c) => {
  ...
  const text = `UPDATE "Gehege" SET name = $1, groesse = $2, kosten = $3 WHERE id = $4`;
...
app.patch("/animals/:id", async (c) => {
  ...
  if (!tier)
    return console.error("Compound not found, dulli.");
  const text = `UPDATE "Tier" SET verpflegungskosten = $1, name = $2, geburtsdatum = $3, geschlecht = $4,art = $5, tierarzt_id = $6, gehege_id = $7, WHERE id = $4`;
  const values = [
    verpflegungskosten,
    name,
    geburtsdatum,
...
app.post("/stand", async (c) => {
  const requestCompounds = await c.req.json();
  const text = `INSERT INTO "Verkaufsstand"(produktart, profit, verkaufer_id) VALUES ($1,$2,$3) RETURNING *`;
  const values = [
